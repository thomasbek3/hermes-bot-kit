...
PIN_401_ERROR = (
    "Orgo rejected the configured API key. Fix ORGO_API_KEY in this "
    "profile's .env (and rotate the key if it is the one exposed on "
    "2026-08-22)."
)
NOT_RUNNING_ERROR = (
...
def _resolve_computer_id() -> str:
    ...
    if isinstance(raw, str) and raw.strip():
        ...
        if COMPUTER_ID_RE.fullmatch(candidate):
            return candidate
        return ""
    env = os.environ.get("ORGO_DEFAULT_COMPUTER_ID", "").strip()
    if COMPUTER_ID_RE.fullmatch(env):
        return env
    return ""
...
def _read_json(response: Any) -> Any:
    try:
        ...
        raise OrgoAgentRequestError(
            "Orgo returned a non-JSON computer-use response."
        ) from exc
...
def pre_tool_call(tool_name: str, args: dict | None = None, **kwargs: Any) -> dict[str, Any] | None:
    """Escalate both computer tools through Hermes' human-approval gate."""
